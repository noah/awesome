...
--
-- from:  http://vincent.bernat.im/en/blog/2012-awesome-wm.html
...
local function markup(keys)
  ...
  for _, key in ipairs(keys) do
    if doc[key] then
      ...
      result[group] = (result[group] or "") ..  '<span color="white"> ' ..  string.format("%" .. (longest - unilen(skey)) .. "s  ", "") .. skey ..  '</span>  <span color="yellow">' ..  help .. '</span>\n'
      -- table.insert(result[group], (result[group] or "") ..  '<span color="white"> ' ..  string.format("%" .. (longest - unilen(skey)) .. "s  ", "") .. skey ..  '</span>  <span color="yellow">' ..  help .. '</span>\n')
    end
  end
...
function display()
  ...
  for group, res in pairs(strings) do
    ...
    result = result ..  '<span weight="bold" color="white">' ..  group .. "</span>\n" .. res
  end
